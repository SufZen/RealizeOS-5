              ...
              <div key={tool.name} className="rounded-lg border border-border p-4">
                ...
                <div className="flex flex-wrap gap-1">
                  {tool.actions.map((a) => (
                    <span
                      key={a}
                      className="text-[10px] px-1.5 py-0.5 rounded bg-surface-700 text-muted-foreground font-mono"
                    >
                      {a}
                    </span>
                  ))}
      ...
      <div className="rounded-xl border border-border bg-card p-6">
        ...
        {!data.google_workspace.gmail && (
          <p className="text-xs text-muted-foreground mt-3">
            <button onClick={() => navigate('/setup')} className="text-brand-400 hover:underline">
              Configure in Setup
            </button>{' '}
    ...
    <div className="rounded-xl border border-border bg-card p-6">
      ...
      {!data || data.events.length === 0 ? (
        <div>
          ...
          <p className="text-xs text-muted-foreground mt-1">
            External services can send events to{' '}
