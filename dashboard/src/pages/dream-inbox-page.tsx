...
const fallbackStatusConfig = statusConfig.pending!
...
function ProposalCard({
  ...
  const status = statusConfig[proposal.status] ?? fallbackStatusConfig
...
export default function DreamInboxPage() {
  ...
  const { data, loading, error, refetch } = useApi<DreamsResponse>('/dreams', 30000, 10000)
  const handleApprove = async (id: string) => {
    await api.post(`/dreams/${id}/approve`)
    refetch()
  }

  const handleReject = async (id: string) => {
    await api.post(`/dreams/${id}/reject`, { reason: '' })
    refetch()
  }
  ...
  const filteredProposals = useMemo(() => {
    ...
    if (cycleFilter) {
      filtered = filtered.filter((p) => p.cycle_type === cycleFilter)
    }
    return [...filtered].sort(
      (a, b) => new Date(b.created_at).getTime() - new Date(a.created_at).getTime(),
    )
  }, [data?.proposals, statusFilter, cycleFilter])
