  ...
  return (
    <div className="rounded-xl border border-border bg-card p-6">
      ...
      <p className="text-xs text-muted-foreground mb-4">
        Re-run the guided tour or onboarding wizard to learn about RealizeOS features
      </p>
...
export function DevModeSection() {
  ...
  useEffect(() => {
    api
      .get<DevModeStatus>('/devmode/status')
      .then((res) => {
        setStatus(res)
  ...
  async function toggleDevMode(enable: boolean) {
    ...
    try {
      ...
      if (res.status === 'ok') {
        setStatus((prev) =>
          prev ? { ...prev, enabled: enable, protection_level: selectedLevel } : prev,
        )
  ...
  async function generateContext() {
    ...
    try {
      const res = await api.post<{ count: number }>('/devmode/setup', {
        protection_level: selectedLevel,
      })
  ...
  return (
    <div className="rounded-xl border border-amber-500/30 bg-card p-6">
      <div className="flex items-center justify-between mb-4">
        ...
        <Toggle checked={status?.enabled ?? false} onChange={(checked) => toggleDevMode(checked)} />
      </div>

      <p className="text-xs text-muted-foreground mb-4">
        Enable integration with local AI coding tools (Claude Code, Gemini CLI, Cursor, etc.) for
        system development and extension authoring.
      </p>
            ...
            <div className="grid grid-cols-2 gap-2">
              {(status.connected_tools || []).length > 0 ? (
                status.connected_tools.map((tool) => (
                  <div
                    key={tool.key}
                    className="flex items-center gap-2 px-3 py-2 rounded-lg bg-surface-800 border border-border"
                  >
                ...
                <p className="text-xs text-muted-foreground col-span-2">
                  No context files generated yet. Click &quot;Generate Context Files&quot; below.
                </p>
              )}
            </div>
          </div>
            ...
            <div className="px-3 py-2 rounded-lg bg-surface-800 border border-border">
              <div className="flex items-center gap-2">
                ...
                <span className="text-xs text-foreground">
                  Last snapshot: {status.last_snapshot.tag}
                </span>
                <span className="text-[10px] text-muted-foreground ml-auto">
                  {status.last_snapshot.timestamp}
                </span>
              </div>
            </div>
          )}
          ...
          {healthResults && (
            <div className="space-y-1">
              ...
              {healthResults.map((check, i) => (
                <div
                  key={i}
                  className="flex items-center gap-2 px-3 py-1.5 rounded bg-surface-800 text-xs"
                >
