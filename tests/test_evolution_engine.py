...
class TestEvolutionEngine:
    ...
    def test_auto_approve_low_risk(self):
        ...
        p = EvolutionProposal(
            id="p1", evolution_type=EvolutionType.NEW_SKILL,
            title="Test", description="", risk_level=RiskLevel.LOW,
        )
        engine.propose(p)
    ...
    def test_reject(self):
        ...
        p = EvolutionProposal(
            id="p1", evolution_type=EvolutionType.CONFIG_CHANGE,
            title="Test", description="", risk_level=RiskLevel.HIGH,
        )
        engine.propose(p)
    ...
    def test_get_pending(self):
        ...
        for i, prio in enumerate([0.3, 0.9, 0.5]):
            engine.propose(EvolutionProposal(
                id=f"p{i}", evolution_type=EvolutionType.NEW_SKILL,
                title=f"Test {i}", description="", priority=prio,
            ))
