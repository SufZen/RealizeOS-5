...
async def run_all():
    async with httpx.AsyncClient(base_url=BASE, timeout=30) as c:
        ...
        if jwt_enabled:
            record("1.4a", "Tampered JWT rejected (JWT enabled)", r.status_code == 401,
                   f"Got {r.status_code}")
        else:
            # JWT not enabled — Bearer tokens should be ignored, request falls through
            record("1.4a", "Tampered JWT ignored (JWT disabled)", r.status_code == 200,
        ...
        record("1.4b", "No auth in dev mode", r.status_code == 200,
               f"Got {r.status_code}")
        ...
        if api_key:
            ...
            record("1.4c", "Correct API key accepted", r_good.status_code == 200,
                   f"Got {r_good.status_code}")

            r_bad = await c.get("/api/dashboard", headers={"X-API-Key": "wrong-key"})
            record("1.4d", "Wrong API key rejected", r_bad.status_code in (401, 403),
                   f"Got {r_bad.status_code}")
        else:
            record("1.4c", "API key auth (not configured)", True,
                   "REALIZE_API_KEY not set — expected in dev mode")
        ...
        record("1.6a", "2MB KB file rejected (PUT)", r.status_code == 413,
               f"Got {r.status_code} (413 = Payload Too Large)")
        ...
        record("1.6b", "500KB KB file accepted (PUT)", r.status_code in (200, 204),
               f"Got {r.status_code}")
        ...
        record("1.6c", "20KB chat message doesn't crash", r.status_code != 500,
        ...
        record("1.8a", "No audit DELETE endpoint", r.status_code in (404, 405),
               f"Got {r.status_code}")
        ...
        record("1.8b", "No audit PUT endpoint", r.status_code in (404, 405),
               f"Got {r.status_code}")
        ...
        record("1.8c", "Audit GET is read-only", r.status_code == 200,
               f"Got {r.status_code}")

        # Verify audit stats endpoint exists
        await asyncio.sleep(1)
        r = await c.get("/api/security/audit/stats")
        record("1.8d", "Audit stats endpoint", r.status_code == 200,
               f"Got {r.status_code}")
